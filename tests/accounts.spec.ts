...
describe("GET /accounts", () => {
  ...
  it("returns the seeded accounts", async () => {
    ...
    for (const account of body) {
      ...
      expect(
        account.renewalDate === null || typeof account.renewalDate === "string"
      ).toBe(true);
...
describe("GET /accounts/:id", () => {
  ...
  it("returns 200 with all domain-model fields for a known id", async () => {
  ...
  it("returns contacts as an empty array for Cetus Agri Co-op", async () => {
    const response = await server.inject({
      method: "GET",
